  ...
  if ("cov" %in% ls(envir=.GlobalEnv)==FALSE) {gmessage("Sorry! Attribute file is not loaded.", parent = window)} else {
    ...
    gbutton("Continue", cont = cg2, width=20, handler = function(h, ...) {
      if (length(dy1)==0 | length(dy4)==0 | length(dy5)==0) {gmessage("Missing required information.", parent = toplevel)} else {
        ...
        dispose(toplevel)
        assign("dy6",rep(1.5,length(dy4)),envir=.GlobalEnv)
        ...
        for (da in 1:length(dy4)) {
          glabel(dy4[da],cont=cge)
        ...
        button <- gbutton("Continue", cont = cge, handler = function(h, ...) {
          ...
          assign("dy6",dy6tmp,envir=.GlobalEnv)
          dispose(extralevel)
          if (length(dy4)==2) {
            single <- function(attr) {
              ...
              gbutton("Continue", cont = cg1, width=20, handler = function(h, ...){
                ...
                button1 <- gbutton("Plot Carrying Capacity", cont = cg2, width=20, handler = function(h, ...) {
                  ...
                  cc3D <- function(za,zb,phi1,theta1) {
                  ...
                  i <- 1
                  tbl[i,1] <- "Z axis min:"
                  tbl[i,2] <- gedit(za,width=5,coerce.with=as.numeric) 
                  tbl[i,3] <- "Z axis max:"
                  tbl[i,4] <- gedit(zb,width=5,coerce.with=as.numeric)
                  tbl[i,5] <- "horizonal angle:"
                  tbl[i,6] <- gedit(0,width=5,coerce.with=as.numeric) 
                  tbl[i,7] <- "vertical angle"
                  tbl[i,8] <- gedit(0,width=5,coerce.with=as.numeric) 
                  tbl[i,9] <- "width"
                  ...
                  button <- gbutton("Refresh Plot", cont = gg, handler = function(h, ...) {
                    Sys.sleep(0.5)
                    cc3D(svalue(tbl[i,2]),svalue(tbl[i,4]),svalue(tbl[i,6]),svalue(tbl[i,8]))
                  })
                ...
                button2 <- gbutton("Plot Membership Rate", cont = cg2, width=20, handler = function(h, ...) {
                  ...
                  mr3D <- function(za,zb,phi1,theta1) {
                    persp3D(x,y,z, facets=F, phi=phi1, theta=theta1,
                                   zlim=c(za,zb),
                  ...
                  button <- gbutton("Refresh Plot", cont = hg, handler = function(h, ...) {
                   Sys.sleep(0.5)
                    mr3D(svalue(tbl[i,2]),svalue(tbl[i,4]),svalue(tbl[i,6]),svalue(tbl[i,8]))
                  })
                ...
                button3 <- gbutton("Plot Intensity of Exploitation", cont = cg2, width=20, handler = function(h, ...) {
                  ...
                  ie3D <- function(za,zb,phi1,theta1) {
                  persp3D(x,y,z, facets=F, phi=phi1, theta=theta1,
                                 zlim=c(za,zb),
                ...
                ang1 <- ang2 <- 0
                za <- min(final$ie_poly)
                zb <- max(final$ie_poly)
                ...
                button <- gbutton("Refresh Plot", cont = ig, handler = function(h, ...) {
                  Sys.sleep(0.5)
                  ie3D(svalue(tbl[i,2]),svalue(tbl[i,4]),svalue(tbl[i,6]),svalue(tbl[i,8]))
                })
                  ...
                  if (dy9!="all") {
                    ...
                    gbutton("Continue", cont = cg1, width=20, handler = function(h, ...){
                      ...
                      for (o in 1:length(t)) {
                         ...
                         cuto1 <- seq(from = dy6[1]/10, to = dy6[1], by = dy6[1]/10)
                         cuto2 <- seq(from = dy6[2]/10, to = dy6[2], by = dy6[2]/10)
                         for(p in 1:length(dy5)) {
