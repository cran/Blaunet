    ...
    gbutton("Continue", expand = FALSE, cont = cg2, handler = function(h, ...) {
      if (length(d1)==0 | length(d4)==0 | length(d5)==0) {gmessage("Missing required information.", parent = toplevel)} else {
        ...
        if (length(d2)>0) {
          ...
          assign("d6",rep(1.5,length(d4)),envir=.GlobalEnv)
          gseparator(cont = extrag)
          glabel("Dev.range",cont=extrag)
          for (da in 1:length(d4)) {
            glabel(d4[da],cont=extrag)
          ...
          button <- gbutton("Continue", expand = FALSE, cont = extrag, handler = function(h, ...) {
            ...
            assign("d6",d6tmp,envir=.GlobalEnv)
            dispose(extralevel)
            if ("all" %in% d9) {
              cov1 <- cov
            ...
            if (length(d4)==2) {
              ...
              niche2d1 <- function(xa,xb,ya,yb) {
                ...
                plot(-10000,-10000,xlim=c(xa,xb),ylim=c(ya,yb), xlab=d4[1], ylab=d4[2])
                if (length(grep('Show nodes',d3))==1) {
                  plot(cov1[,k2[1]],cov1[,k2[2]],xlim=c(xa,xb),ylim=c(ya,yb), xlab=d4[1], ylab=d4[2], col="red")
                }
                bcol <- 1
              ...
              sndlevel <- gwindow("2D Niche Plot", width = 800, height = 800)
              ...
              tbl <- glayout(cont = sg,spacing=5)
              i <- 1
              tbl[i,1] <- "X axis min:"
              tbl[i,2] <- gedit(xa,width=5,coerce.with=as.numeric) 
              tbl[i,3] <- "X axis max:"
              tbl[i,4] <- gedit(xb,width=5,coerce.with=as.numeric)
              tbl[i,5] <- "Y axis min:"
              tbl[i,6] <- gedit(ya,width=5,coerce.with=as.numeric) 
              tbl[i,7] <- "Y axis max:"
              tbl[i,8] <- gedit(yb,width=5,coerce.with=as.numeric) 
              tbl[i,9] <- "width"
              tbl[i,10] <- gcombobox(c(600,800,1000,1200,1400,1600,1800,2000), selected = 1, cont = tbl, 
                handler = function(h,...){assign("o1",svalue(h$obj),envir=.GlobalEnv)})
              tbl[i,11] <- "height"
              tbl[i,12] <- gcombobox(c(600,800,1000,1200,1400,1600,1800,2000), selected = 1, cont = tbl, 
                handler = function(h,...){assign("o2",svalue(h$obj),envir=.GlobalEnv)})
              tbl[i,13] <- "DPI"
              tbl[i,14] <- gcombobox(c(100,200,300,400,500,600), selected = 1, cont = tbl, 
                handler = function(h,...){assign("o3",svalue(h$obj),envir=.GlobalEnv)})
              tbl[i,15] <- "format"
              tbl[i,16] <- gcombobox(c("png","jpg","bmp","tiff","pdf"), selected = 1, cont = tbl, 
                handler = function(h,...){assign("o4",svalue(h$obj),envir=.GlobalEnv)})
              button <- gbutton("Refresh Plot", cont = sg, handler = function(h, ...) {
                 niche2d1(svalue(tbl[i,2]),svalue(tbl[i,4]),svalue(tbl[i,6]),svalue(tbl[i,8]))
              })
              ...
              niche3d <- function(xa,xb,ya,yb,za,zb,phi1,theta1) {
                add_legend <- function(...) {
                  opar <- par(fig=c(0, 1, 0, 1), oma=c(0, 0, 0, 0), 
                  mar=c(0, 0, 0, 0), new=TRUE)
                  on.exit(par(opar))
              ...
              ya <- min(lowb[,2],cov1[,k2[2]],na.rm = TRUE)
              yb <- max(topb[,2],cov1[,k2[2]],na.rm = TRUE)
              za <- min(lowb[,3],cov1[,k2[3]],na.rm = TRUE)
              zb <- max(topb[,3],cov1[,k2[3]],na.rm = TRUE)
              ang1 <- ang2 <- 0
              ...
              tbl[i,8] <- gedit(yb,width=5,coerce.with=as.numeric) 
              tbl[i,9] <- "Z axis min:"
              tbl[i,10] <- gedit(za,width=5,coerce.with=as.numeric) 
              tbl[i,11] <- "Z axis max:"
              tbl[i,12] <- gedit(zb,width=5,coerce.with=as.numeric) 
              tbl[i,13] <- "horizonal angle:"
              tbl[i,14] <- gedit(0,width=5,coerce.with=as.numeric) 
              tbl[i,15] <- "vertical angle"
              tbl[i,16] <- gedit(0,width=5,coerce.with=as.numeric) 
              tbl[i,17] <- "width"
              tbl[i,18] <- gcombobox(c(600,800,1000,1200,1400,1600,1800,2000), selected = 1, cont = tbl, 
                handler = function(h,...){assign("n3d1",svalue(h$obj),envir=.GlobalEnv)})
              tbl[i,19] <- "height"
              tbl[i,20] <- gcombobox(c(600,800,1000,1200,1400,1600,1800,2000), selected = 1, cont = tbl, 
                handler = function(h,...){assign("n3d2",svalue(h$obj),envir=.GlobalEnv)})
              tbl[i,21] <- "DPI"
              tbl[i,22] <- gcombobox(c(100,200,300,400,500,600), selected = 1, cont = tbl, 
                handler = function(h,...){assign("n3d3",svalue(h$obj),envir=.GlobalEnv)})
              tbl[i,23] <- "format"
              tbl[i,24] <- gcombobox(c("png","jpg","bmp","tiff","pdf"), selected = 1, cont = tbl, 
                handler = function(h,...){assign("n3d4",svalue(h$obj),envir=.GlobalEnv)})
              button <- gbutton("Refresh Plot", cont = fg, handler = function(h, ...) {
                 niche3d(svalue(tbl[i,2]),svalue(tbl[i,4]),svalue(tbl[i,6]),svalue(tbl[i,8]),svalue(tbl[i,10]),svalue(tbl[i,12]),svalue(tbl[i,14]),svalue(tbl[i,16]))
              })
              ...
              dev1 <- dev.cur();
              Sys.sleep(0.5)
              niche3d(xa,xb,ya,yb,za,zb,ang1,ang2)
            } else {
              dispose(toplevel)
          ...
          button <- gbutton("Continue", expand = FALSE, cont = extrag, handler = function(h, ...) {
            ...
            if (length(d4)==2) {
              ...
              niche2d2 <- function(xa,xb,ya,yb) {
                ...
                plot(-10000,-10000,xlim=c(xa,xb),ylim=c(ya,yb), xlab=d4[1], ylab=d4[2])
                if (length(grep('Show nodes',d3))==1) {
                  plot(cov[,k2[1]],cov[,k2[2]],xlim=c(xa,xb),ylim=c(ya,yb), xlab=d4[1], ylab=d4[2],col="red")
                }
              ...
              sndlevel <- gwindow("2D Niche Plot", width = 800, height = 800)
              sg <- ggroup(cont = sndlevel, horizontal = F, expand=T)
              xa <- min(lowb[,1],cov[,k2[1]],na.rm = TRUE)
              xb <- max(topb[,1],cov[,k2[1]],na.rm = TRUE)
              ya <- min(lowb[,2],cov[,k2[2]],na.rm = TRUE)
              yb <- max(topb[,2],cov[,k2[2]],na.rm = TRUE)
              assign("o1",600,envir=.GlobalEnv)
              assign("o2",600,envir=.GlobalEnv)
              assign("o3",100,envir=.GlobalEnv)
              assign("o4","png",envir=.GlobalEnv)
              ...
              tbl[i,16] <- gcombobox(c("png","jpg","bmp","tiff","pdf"), selected = 1, cont = tbl, 
                handler = function(h,...){assign("o4",svalue(h$obj),envir=.GlobalEnv)})
              button <- gbutton("Refresh Plot", cont = sg, handler = function(h, ...) {
                 niche2d2(svalue(tbl[i,2]),svalue(tbl[i,4]),svalue(tbl[i,6]),svalue(tbl[i,8]))
              })        
              button <- gbutton("Save Plot", cont = sg, handler = function(h, ...) {
                if (o4=="png") {
                  png("niche2d.png", width = o1, height = o2, res = o3)
              ...
              ya <- min(lowb[,2],cov[,k2[2]],na.rm = TRUE)
              yb <- max(topb[,2],cov[,k2[2]],na.rm = TRUE)
              za <- min(lowb[,3],cov[,k2[3]],na.rm = TRUE)
              zb <- max(topb[,3],cov[,k2[3]],na.rm = TRUE)
              ang1 <- ang2 <- 0
              assign("n3d1",600,envir=.GlobalEnv)
              assign("n3d2",600,envir=.GlobalEnv)
              assign("n3d3",100,envir=.GlobalEnv)
